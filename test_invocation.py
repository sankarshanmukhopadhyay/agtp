...
from agtp.client import build_parser as build_client_parser
from agtp.curl import build_parser as build_curl_parser
from agtp.curl import run as curl_run
...
def _run_server(args: List[str]) -> Tuple[subprocess.Popen, int]:
    """Spawn `python -m agtp.server <args>` from the repo root."""
    proc = subprocess.Popen(
        [PYTHON, "-m", "agtp.server", *args],
        cwd=str(REPO_ROOT),
...
class ArgParsingTests(unittest.TestCase):
    ...
    def test_server_accepts_positional_port(self) -> None:
        from agtp.server import main as server_main
    ...
    def test_server_rejects_both_positional_and_flag(self) -> None:
        out = subprocess.run(
            [PYTHON, "-m", "agtp.server", "4480", "--port", "4480"],
            cwd=str(REPO_ROOT),
            capture_output=True,
            text=True,
            timeout=10,
    ...
    def test_server_rejects_non_loopback_without_tls_or_insecure(self) -> None:
        out = subprocess.run(
            [PYTHON, "-m", "agtp.server", "0", "--host", "0.0.0.0"],
            cwd=str(REPO_ROOT),
            capture_output=True,
            text=True,
            timeout=10,
...
class CurlAgainstLiveServerTests(unittest.TestCase):
    """Spawn a real `python -m agtp.server` and hit it with agtp-curl."""
    ...
    def setUpClass(cls) -> None:
        ...
        src = REPO_ROOT / "v1" / "server" / "agents" / "lauren.agent.json"
