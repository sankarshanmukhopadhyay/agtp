...
| 10b | INSPECT audit read surface | Implemented (Phase 6); embedded verb #13. `target=audit` returns the signed JWS for a given `audit_id`; `target=chain_head` returns the latest `audit_id` for a given `agent_id`; `target=lifecycle` returns an agent's full lifecycle event stream (Phase 8). JWSes persisted under `[audit].records_root` (default `~/.agtp/audit/records/`, sharded by 2-char hex prefix); lifecycle events under `[audit].lifecycle_root` (default `~/.agtp/audit/lifecycle/`, one JSONL per agent). Powers the chain inspector at `tools/chain_inspector/`. |
| 10c | Identity lifecycle methods | Implemented (Phase 8); embedded verbs ACTIVATE, DEACTIVATE, REVOKE. Each updates the targeted AgentDocument's `status` and emits a signed lifecycle event into the agent's `audit/lifecycle/{agent_id}.jsonl` stream. Open auth in v1 — every caller can transition any agent; the audit trail is the accountability mechanism. SCITT (RFC 9943 COSE_Sign1) is reserved via `[audit].mode = scitt` but the daemon refuses to boot in scitt mode today with a clear "future work" message. |
...
| 17 | AGTP-LOG receipt emission | Implemented as Ed25519-signed JSONL (`mod_audit`). Lifecycle stream supports two on-disk forms via `[audit].mode`: `jws` (default, JWS Compact Attribution-Record) and `scitt` (RFC 9943 COSE_Sign1, `cose:` prefix, consumable by any SCITT verifier). Same Ed25519 key signs both; INSPECT lifecycle reader handles mixed streams. |
