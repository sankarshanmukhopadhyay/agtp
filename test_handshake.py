...
from agtp.config import AgentsConfig, ServerConfig, ServerInfo, ServerPolicy
from agtp.handshake import MatchOutcome, format_outcome, match, match_from_manifest_dict
from agtp.identity import AgentDocument, RequiresDeclaration, from_dict
...
def _read_agent(name: str) -> AgentDocument:
    return from_dict(json.loads(
        (REPO_ROOT / "v1" / "server" / "agents" / name).read_text(encoding="utf-8")
    ))
...
class SoftDenyEnforcementTests(unittest.TestCase):
    ...
    def test_soft_deny_452_for_undeclared_cognitive_method(self):
        ...
        try:
            ...
            self.assertEqual(payload["error"]["code"], "method-outside-need")
...
class MatchCheckCLITests(unittest.TestCase):
    ...
    def test_match_check_against_live_server(self):
        # Spin up `python -m agtp.server` on a free port and run the
        ...
        agents_dir = REPO_ROOT / "v1" / "server" / "agents"
        proc = subprocess.Popen(
            [
                PYTHON, "-m", "agtp.server", str(port),
                "--host", "127.0.0.1",
