...
LEGACY_DIR = REPO_ROOT / "v1" / "server" / "agents" / "legacy"
LIVE_DIR = REPO_ROOT / "v1" / "server" / "agents"
...
class MigrateCLITests(unittest.TestCase):
    ...
    def test_check_mode_does_not_modify(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            out = subprocess.run(
                [sys.executable, "-m", "agtp.migrate", "--check", str(target)],
                cwd=str(REPO_ROOT),
                capture_output=True,
                text=True,
                timeout=10,
    ...
    def test_migrates_in_place_with_backup(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            out = subprocess.run(
                [sys.executable, "-m", "agtp.migrate", str(target)],
                cwd=str(REPO_ROOT),
                capture_output=True,
                text=True,
                timeout=10,
